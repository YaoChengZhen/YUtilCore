...
let modularName = "YUtilCore"

public  class YUtilCore {
    public static var sharedInstance : YUtilCore {
        struct Static {
            static let instance : YUtilCore = YUtilCore()
        }
    ...
    static public var alertmsg:Dictionary<Int,(msgcode:Int,msgtitle:String,msginfo:String)>{
        get{
            for dict  in YUtilCore.msg {
                alert_msg[dict.0] = dict.1
            }
            return alert_msg
        }
        set (newValue){
            YUtilCore.msg = newValue
        }
    }
    /// 登陆模块的storyboard
    public static var storyboard:UIStoryboard{
        get{
            return UIStoryboard(name: modularName, bundle: YUtilCore.bundle)
        }
    }
    /// 模块中的 扫一扫vc
    public static var scanVc:Scan_Vc{
        get{
            return YUtilCore.storyboard.instantiateViewController(withIdentifier: "Scan_Vc") as! Scan_Vc
        }
    }
    ///供其他模块使用
