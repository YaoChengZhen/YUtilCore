...
extension UIView {
    ...
    public func toastErrorCode(_ errorCode: Int){
        self.toast(YUtilCore.alertmsg[errorCode]?.msgtitle ?? "")
    ...
    public func toastCenterErrorCode(_ errorCode: Int){
        self.toastCenter(YUtilCore.alertmsg[errorCode]?.msgtitle ?? "")
    ...
    public func toastBottomErrorCode(_ errorCode: Int){
        self.toastBottom(YUtilCore.alertmsg[errorCode]?.msgtitle ?? "")
    ...
    public func toastError(_ error:MikerError){
        if error.code == YUtilCore.sharedInstance.toLoginErrorCode {
            self.toastCompletion(error.message){ _ in
                 ...
                 */
                if YUtilCore.sharedInstance.isHasToLoginVc == false {
                    _ = "login".openURL()
                }
            }
        }else if error.code == YUtilCore.sharedInstance.toForcedupdatingErrorCode{
            /*
            表示版本强制更新
             */
            if YUtilCore.sharedInstance.isHasForcedupdating == false {
                YUtilCore.sharedInstance.isHasForcedupdating = true
                _ = "forcedupdating".openURL(["message":error.message])
            }
            
        }else{
            if YUtilCore.sharedInstance.isDebug {
                ...
                if code.hasPrefix("2") {
                    ...
                    self.toast(YUtilCore.sharedInstance.errorMsg)
                }
            }
        }
    }
        ...
        if error.code == YUtilCore.sharedInstance.toLoginErrorCode {
            ...
            if YUtilCore.sharedInstance.isDebug {
                ...
                if code.hasPrefix("2") {
                    ...
                    self.toastCompletion(YUtilCore.sharedInstance.errorMsg, completion: completion)
                }
            }
        }
    }
    ...
    public var rx_showerrorpage: AnyObserver<MikerError> {
        return Binder(self) { baseVc, error in
            ...
            if code.hasPrefix("2") {
                ...
                if let emptyView = baseVc.view.viewWithTag(YUtilCore.ErrorPageViewTag) as? ErrorPageView {
